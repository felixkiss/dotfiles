#
# Inspired by https://github.com/ohmyzsh/ohmyzsh/blob/master/plugins/timer/timer.plugin.zsh
#
...
__timer_display_timer_precmd() {
  _PROMPT_LAST_COMMAND_RETURN_CODE="$?"
  if [ -n "${__timer_cmd_start_time}" ]; then
    ...
    if [[ -z "${TIMER_THRESHOLD}" || ${tdiff} -ge "${TIMER_THRESHOLD}" ]]; then
      ...
      _PROMPT_LAST_COMMAND_DURATION="$tdiffstr"
    else
